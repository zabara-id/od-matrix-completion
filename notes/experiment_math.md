...
Энтропийный зеркальный шаг (exponentiated gradient) имеет вид $D\mapsto D\odot\exp(-\eta G)$ и сохраняет $D_{ij}\ge 0$ (в отличие от обычного аддитивного шага градиентного спуска, который легко даёт отрицательные значения и требует дополнительных хаков).
